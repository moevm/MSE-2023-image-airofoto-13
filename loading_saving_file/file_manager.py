...
class FileManager(IFileManager):
    ...
    def __init__(self, parsers: Dict[str, IParser] = None):
        ...
        if parsers is None:
            ...
            parsers = {"yml": YmlParser.create_parser(), "ply": PlyParser.create_parser()}

        array_keys = parsers.keys()
        if "yml" not in array_keys:
            parsers.update({"yml": YmlParser.create_parser()})
        if "ply" not in array_keys:
            parsers.update({"ply": PlyParser.create_parser()})
